...
def run_explain_marker(count_sql: str) -> Tuple[bool, str]:
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("EXPLAIN (VERBOSE, COSTS OFF) " + count_sql.strip().rstrip(";") + ";")
...
def main() -> int:
    ...
    for qid in map(str, range(1, 23)):
        ...
        ok_marker, plan = run_explain_marker(count_sql)
        if not ok_marker:
        ...
        try:
            ...
            fail("ours_error", f"q{qid}: {msg}", repro_sql=None, explain_sql=count_sql)
        ...
        try:
            ...
            msg = (getattr(exc, "pgerror", None) or str(exc)).replace("\n", " ").strip()
            fail("rls_error", f"q{qid}: {msg}", repro_sql=None, explain_sql=count_sql)
