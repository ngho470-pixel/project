...
OUT_CSV = REPO_ROOT / 'logs' / 'pf_v2_6_perf.csv'
OUT_MD = REPO_ROOT / 'logs' / 'pf_v2_6_perf.md'
...
POLICIES: Tuple[Policy, ...] = (
    Policy('P26', "1. orders: orders.o_custkey = customer.c_custkey AND (orders.o_totalprice >= customer.c_acctbal)", 'orders'),
    Policy('P27', "3. lineitem: lineitem.l_orderkey = orders.o_orderkey AND (lineitem.l_extendedprice <= orders.o_totalprice)", 'lineitem'),
    Policy('P28', "5. partsupp: partsupp.ps_partkey = part.p_partkey AND (part.p_retailprice >= partsupp.ps_supplycost)", 'partsupp'),
    Policy('P30', "7. supplier: supplier.s_nationkey = customer.c_nationkey AND (supplier.s_acctbal >= customer.c_acctbal)", 'supplier'),
    Policy('P27F', "9. lineitem: lineitem.l_orderkey = orders.o_orderkey AND orders.o_orderstatus = 'F' AND (lineitem.l_extendedprice <= orders.o_totalprice)", 'lineitem'),
    Policy('P62', "11. lineitem: lineitem.l_partkey = partsupp.ps_partkey AND partsupp.ps_suppkey = supplier.s_suppkey AND (lineitem.l_suppkey <= supplier.s_suppkey)", 'lineitem'),
)
...
def run_count_hash(db: str, baseline: str, qid: str, qsql: str, enabled_path: Path, timeout_ms: int):
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            if baseline in ('rls','rls_with_index'): cur.execute('SET row_security = on;')
...
def main() -> None:
    import argparse
    ap = argparse.ArgumentParser(description='PF-V2.6 perf runner (curated comparator cases incl arity-2)')
    ...
    args = ap.parse_args()
    db = str(args.db); timeout_ms = h.parse_timeout_ms(args.statement_timeout)
    queries = qmap(); write_policy_file()
    cols = [
        'db','policy_code','policy_target','query_id','enabled_policy_id','ours_hot_avg_ms','rls_hot_avg_ms','ours_vs_rls_ratio',
        'ours_hot_avg_peak_rss_kb','rls_hot_avg_peak_rss_kb','correctness','ours_rows','ours_hash','gt_rows','gt_hash',
        'policy_total_ms','project_ms','project_row_ms','pf2_terms_tree','pf2_cmp_total','pf2_cmp_supported','pf2_cmp_key_arity_max',
        ...
        'pf2_tree_edges','pf2_tree_rows_scanned_total','pf2_tree_update_ms','pf2_project_ms','pf2_total_ms',
        'proj_sig_count','proj_mask_or_ops','proj_rid_iters','status','error_type','error_msg'
    ]
    rows: List[Dict[str,str]] = []
    for idx, c in enumerate(CASES, start=1):
        p = pol(c.policy_code); qid = c.query_id; qsql = queries[qid]
        pid = h.parse_policy_entry_with_id(p.line)[0] or 0
        enabled = Path('/tmp') / f'pf_v2_6_perf_enabled_{p.code}_q{qid}.txt'
        enabled.parent.mkdir(parents=True, exist_ok=True)
        h.write_enabled_policy_file([p.line], enabled)
        os.chmod(enabled, 0o644)
        row = {k:'' for k in cols}
        row.update({'db':db,'policy_code':p.code,'policy_target':p.target,'query_id':qid,'enabled_policy_id':str(pid),'status':'ok','error_type':'','error_msg':''})
        try:
            ...
            if None in (oc, oh, gc, gh):
                row.update({'correctness':'ERROR','status':'error','error_type':'count_hash','error_msg':f"ours={oerr if oc is None else ''} gt={gerr if gc is None else ''}".strip()})
            else:
                row.update({'ours_rows':str(oc),'ours_hash':str(oh),'gt_rows':str(gc),'gt_hash':str(gh)})
                if oc == gc and oh == gh: row['correctness'] = 'PASS'
                else: row.update({'correctness':'FAIL','status':'error','error_type':'mismatch','error_msg':'rows/hash mismatch'})
            ...
            row.update({'status':'error','error_type':'exception','error_msg': (getattr(exc,'pgerror',None) or str(exc)).replace('\n',' ').strip()[:320]})
            try: h.clear_artifacts(db)
            except Exception: pass
            try: h.clear_rls_indexes_and_policies(db)
            except Exception: pass
        rows.append(row)
        print(f"[pf2.6_perf] {p.code} q{qid} status={row['status']} correctness={row.get('correctness','')}")
    ...
    lines = [
        '# PF-V2.6 Perf','',f'- db: `{db}`',f'- policy_file: `{POLICY_FILE}`',
        '- mode: `strict_mode=on`',
        f'- cases: {len(rows)}','', '## Per Case'
    ]
    for r in rows:
        lines.append(
            f"- {r['policy_code']} target={r['policy_target']} q{r['query_id']}: status={r['status']} correctness={r.get('correctness','')} ours={r.get('ours_hot_avg_ms','')}ms rls={r.get('rls_hot_avg_ms','')}ms ours/rls={r.get('ours_vs_rls_ratio','')} policy_total={r.get('policy_total_ms','')} project={r.get('project_ms','')} pf2_tree={r.get('pf2_terms_tree','')} pf2_cmp={r.get('pf2_cmp_total','')}/{r.get('pf2_cmp_supported','')} cmp_build_ms={r.get('pf2_cmp_summary_build_ms','')} key2(entries/build/lookups)={r.get('pf2_cmp_key2_entries','')}/{r.get('pf2_cmp_key2_build_ms','')}/{r.get('pf2_cmp_key2_lookups','')} cmp_checks/rejects={r.get('pf2_cmp_checks_total','')}/{r.get('pf2_cmp_rejects_total','')} tree_edges={r.get('pf2_tree_edges','')} tree_rows={r.get('pf2_tree_rows_scanned_total','')} tree_update_ms={r.get('pf2_tree_update_ms','')} proj_sig={r.get('proj_sig_count','')} proj_mask_or={r.get('proj_mask_or_ops','')} proj_rid={r.get('proj_rid_iters','')}"
        )
    if med is not None:
