...
OUT_MD = REPO_ROOT / 'logs' / 'class_ww_cmp_shape_fail.md'
...
CASES = (
    Case(
        'F_WW_NONADJ',
        'non-adjacent witness-witness comparator must fail-loud',
        '1. lineitem: lineitem.l_orderkey = orders.o_orderkey AND orders.o_custkey = customer.c_custkey AND customer.c_nationkey = nation.n_nationkey AND (orders.o_totalprice >= nation.n_regionkey)',
        '1',
    ),
)
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description='Class-engine witness-witness comparator fail-loud checks')
    ...
    for idx, c in enumerate(CASES, start=1):
        enabled = Path('/tmp') / f'class_ww_cmp_shape_fail_{c.code}.txt'
        ...
        try:
            ...
            with strict_env():
                ...
                m, _payload, _kv, _cnt, _notices = h.run_ours_profile_capture(
                    ...
                    query_id=c.query_id,
                    profile_k=idx,
                    profile_query=c.query_id,
                )
            msg = (m.error_msg or '') if m.status == 'error' else ''
            if m.status == 'error' and ('unsupported term shape' in msg or 'unsupported' in msg):
                status, err = 'PASS', msg
            else:
                status, err = 'FAIL', (msg or 'query unexpectedly succeeded under strict mode')
        except Exception as exc:  # noqa: BLE001
            err = (getattr(exc, 'pgerror', None) or str(exc)).replace('\n', ' ').strip()[:350]
            status = 'PASS' if ('unsupported term shape' in err or 'unsupported' in err) else 'FAIL'
        finally:
            try:
                h.clear_artifacts(db)
            except Exception:
        ...
        print(f"[class_ww_cmp_shape_fail] {c.code} status={status}")
