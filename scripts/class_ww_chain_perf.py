...
OUT_CSV = REPO_ROOT / 'logs' / 'class_ww_chain_perf.csv'
OUT_MD = REPO_ROOT / 'logs' / 'class_ww_chain_perf.md'
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description='Class-engine witness-witness chain comparator perf runner')
    ...
    cols = [
        ...
        'class_route_tree', 'class_route_cycle_rect',
    ...
    for idx, c in enumerate(CASES, start=1):
        ...
        enabled = Path('/tmp') / f'class_ww_chain_perf_{c.policy_code}_{c.query_id}.txt'
        ...
        try:
            ...
            for invk in ('proj_sig_count', 'proj_mask_or_ops', 'proj_rid_iters'):
                ...
                if v not in ('', '0', '0.000'):
                    ...
                    row['error_msg'] = f'invariant {invk}={v}'

            if row['status'] == 'ok':
                if str(row.get('class_route_tree', '0')) in ('', '0'):
                    row['status'] = 'error'
                    row['error_type'] = 'route'
                    row['error_msg'] = 'missing class_route_tree'
                if str(row.get('pf2_cmp_chain_total', '0')) in ('', '0'):
                    row['status'] = 'error'
                    row['error_type'] = 'counter'
                    row['error_msg'] = 'missing pf2_cmp_chain_total'
        ...
        rows.append(row)
        print(f'[class_ww_chain_perf] case={c.policy_code} q{c.query_id} status={row["status"]} correctness={row.get("correctness","")}')
    ...
    lines = [
        ...
        '- mode: `strict_mode=on`',
        f'- cases: {len(rows)}',
        f'- median ours/rls hot-time ratio: {med_ratio:.3f} (target <= 0.85)',
        '',
        '## Cases',
    ]
    for r in rows:
        lines.append(
            ...
            f"status={r['status']} correctness={r.get('correctness','')} "
