...
OUT_CSV = REPO_ROOT / 'logs' / 'class_ww_cmp_perf.csv'
OUT_MD = REPO_ROOT / 'logs' / 'class_ww_cmp_perf.md'
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description='Class-engine witness-witness comparator perf runner')
    ...
    for idx, c in enumerate(CASES, start=1):
        ...
        enabled = Path('/tmp') / f'class_ww_cmp_perf_{c.policy_code}_{c.query_id}.txt'
        ...
        rows.append(row)
        print(f"[class_ww_cmp_perf] case={p.code} q{c.query_id} status={row['status']} correctness={row.get('correctness','')}")
    ...
    ratios = [float(r['ours_vs_rls_ratio']) for r in rows if r.get('status') == 'ok' and r.get('ours_vs_rls_ratio')]
    ...
    md_lines = [
        ...
        '- mode: `strict_mode=on`',
        f'- cases: {len(rows)}',
        f"- median ours/rls hot-time ratio: {med_ratio:.3f} (target <= {target_ratio})" if ratios else '- median ours/rls hot-time ratio: n/a',
        '',
        '## Cases',
    ]
    for r in rows:
