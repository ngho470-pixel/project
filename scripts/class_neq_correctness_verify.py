...
LOG_CSV = REPO_ROOT / 'logs' / 'class_neq_correctness.csv'
LOG_MD = REPO_ROOT / 'logs' / 'class_neq_correctness.md'
...
POLICIES: Tuple[Policy, ...] = (
    Policy('N1', "1. lineitem: lineitem.l_returnflag != 'R'", 'lineitem', 'same-table const !='),
    Policy('N2', '3. lineitem: lineitem.l_commitdate != lineitem.l_receiptdate', 'lineitem', 'same-table col-col !='),
    Policy('N3', '5. orders: orders.o_custkey = customer.c_custkey AND (orders.o_custkey != customer.c_custkey)', 'orders', 'cross-table key1 != impossible sanity (expect empty)'),
    Policy('N4', '7. orders: orders.o_custkey = customer.c_custkey AND (orders.o_totalprice != customer.c_acctbal)', 'orders', 'cross-table key1 != realistic'),
    Policy('N5', '9. lineitem: lineitem.l_partkey = partsupp.ps_partkey AND lineitem.l_suppkey = partsupp.ps_suppkey AND (lineitem.l_quantity != partsupp.ps_availqty)', 'lineitem', 'cycle-rect key2 !='),
)
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description='Class-engine != correctness verifier')
    ...
    for idx, c in enumerate(CASES, start=1):
        ...
        enabled = Path('/tmp') / f'class_neq_correctness_{c.policy_code}_{c.query_id}.txt'
        ...
        rows.append(row)
        print(f"[class_neq_correctness] case={p.code} q{c.query_id} status={row['status']}")
