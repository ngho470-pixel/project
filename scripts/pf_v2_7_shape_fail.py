...
OUT_MD = REPO_ROOT / 'logs' / 'pf_v2_7_shape_fail.md'
...
CASES = (
    Case('F_XNE', 'cross-table != unsupported in PF-V2.7 cycle path',
         "1. lineitem: lineitem.l_partkey = partsupp.ps_partkey AND lineitem.l_suppkey = partsupp.ps_suppkey AND (lineitem.l_quantity != partsupp.ps_availqty)", '6'),
    Case('F_CYCLE_EXTRA', 'cycle with extra table unsupported in PF-V2.7 cycle-rectangle subset',
         "3. lineitem: lineitem.l_partkey = partsupp.ps_partkey AND lineitem.l_suppkey = partsupp.ps_suppkey AND partsupp.ps_partkey = part.p_partkey AND (lineitem.l_quantity <= partsupp.ps_availqty)", '6'),
    Case('F_KGT2', 'comparator separator arity >2 unsupported under PF force',
         "5. supplier: supplier.s_suppkey = lineitem.l_suppkey AND lineitem.l_orderkey = orders.o_orderkey AND orders.o_custkey = customer.c_custkey AND (customer.c_acctbal <= supplier.s_acctbal)", '5'),
)
...
def main() -> None:
    import argparse
    ap = argparse.ArgumentParser(description='PF-V2.7 fail-loud shape checks')
    ...
    for idx, c in enumerate(CASES, start=1):
        enabled = Path('/tmp') / f'pf_v2_7_shape_fail_{c.code}.txt'
        ...
        try:
            ...
            if m.status == 'error' and 'PF-V2 path unsupported term shape' in msg:
                ...
                status, err = 'FAIL', (msg or 'query unexpectedly succeeded under PF force')
        ...
        print(f"[pf2.7_shape_fail] {c.code} status={status}")
