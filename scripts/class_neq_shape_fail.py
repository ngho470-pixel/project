...
OUT_MD = REPO_ROOT / 'logs' / 'class_neq_shape_fail.md'
...
CASES = (
    Case(
        'F_NEQ_K3',
        'cross-table != with separator key arity > 2 must fail-loud',
        '1. supplier: supplier.s_suppkey = lineitem.l_suppkey AND lineitem.l_orderkey = orders.o_orderkey AND orders.o_custkey = customer.c_custkey AND (supplier.s_acctbal != customer.c_acctbal)',
        '5',
    ),
)
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description='Class-engine != fail-loud shape checks')
    ...
    for idx, c in enumerate(CASES, start=1):
        enabled = Path('/tmp') / f'class_neq_shape_fail_{c.code}.txt'
        ...
        print(f"[class_neq_shape_fail] {c.code} status={status}")
